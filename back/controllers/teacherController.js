...
const evaluateCompletionRequest = async (req, res) => {
  try {
    ...
    const { teacher_comments, teacher_rating, status, detailed_evaluation_data } = req.body;
    const teacherId = req.user.id;

    console.log('🔵 Backend - Evaluating completion request:', { requestId, teacher_comments, teacher_rating, status, teacherId });
    console.log('🔵 Backend - Detailed evaluation data:', JSON.stringify(detailed_evaluation_data, null, 2));
    ...
    try {
      ...
      if (detailed_evaluation_data && detailed_evaluation_data.length > 0) {
        ...
        console.log('🔵 Backend - Detailed evaluation data processed successfully');
      }
      ...
      const updateQuery = `
        ...
        WHERE id = ?
      `;
      console.log('🔵 Backend - Updating completion request with:', {
        teacher_comments,
        teacher_rating,
        finalStatus,
    ...
    console.error('🔴 Backend - Error evaluating completion request:', error);
...
const getMySchoolInfo = async (req, res) => {
  try {
    ...
    const schoolQuery = `
      SELECT s.id, s.school_id, s.school_name as name, s.address, s.phone, s.created_at, s.updated_at, st.academic_year_id
      FROM schools s
...
const getAllStudentsLessonPlans = async (req, res) => {
  try {
    const teacherId = req.user.id;
    const { page = 1, limit = 10, search = '', subject_id = '', status = '' } = req.query;
    
    // ดึงข้อมูลครูพี่เลี้ยงและโรงเรียน
    const teacher = await User.findById(teacherId);
    if (!teacher) {
      return res.status(404).json({
        success: false,
    ...
    // ดึงข้อมูลโรงเรียนของครูพี่เลี้ยง
    const schoolInfo = await SchoolInfo.getTeacherSchoolInfo(teacherId);
    if (!schoolInfo) {
      return res.status(404).json({
        success: false,
    ...
    // ดึงข้อมูลแผนการสอนของนักเรียนทั้งหมดในโรงเรียนเดียวกัน
    const result = await LessonPlan.getBySchoolWithStudents(
    ...
    console.error('Error getting all students lesson plans:', error);
    res.status(500).json({
      success: false,
      message: 'เกิดข้อผิดพลาดในการดึงข้อมูลแผนการสอน'
    });
...
// ดึงข้อมูลบันทึกการฝึกประสบการณ์ของนักเรียนทั้งหมดในโรงเรียนเดียวกัน
const getAllStudentsTeachingSessions = async (req, res) => {
  try {
    const teacherId = req.user.id;
    const { page = 1, limit = 10, search = '', subject_id = '', status = '' } = req.query;
    ...
    // ดึงข้อมูลบันทึกการฝึกประสบการณ์ของนักเรียนทั้งหมดในโรงเรียนเดียวกัน
    const result = await TeachingSession.getBySchoolWithStudents(
      schoolInfo.school_id,
      parseInt(page),
      parseInt(limit),
      search,
    ...
    res.json({
      success: true,
      data: result.teachingSessions,
      pagination: result.pagination
    });
  } catch (error) {
    console.error('Error getting all students teaching sessions:', error);
    res.status(500).json({
      success: false,
      message: 'เกิดข้อผิดพลาดในการดึงข้อมูลบันทึกการฝึกประสบการณ์'
    });
  }
};

// ดาวน์โหลดไฟล์แผนการสอน
const downloadLessonPlanFile = async (req, res) => {
  try {
    const { planId, fileId } = req.params;
    const teacherId = req.user.id;

    // ตรวจสอบว่าไฟล์นี้อยู่ในโรงเรียนของครูหรือไม่
    const LessonPlan = require('../models/LessonPlan');
    const SchoolInfo = require('../models/SchoolInfo');
    ...
    const [files] = await pool.execute(
      `SELECT lpf.*, lp.student_id, ia.school_id 
       FROM lesson_plan_files lpf
       JOIN lesson_plans lp ON lpf.lesson_plan_id = lp.id
       JOIN internship_assignments ia ON lp.student_id = ia.student_id
       WHERE lpf.id = ? AND lpf.lesson_plan_id = ? AND ia.school_id = ?`,
      [fileId, planId, schoolInfo.school_id]
    );
    ...
    if (!fs.existsSync(file.file_path)) {
      return res.status(404).json({ success: false, message: 'ไฟล์ไม่พบในระบบ' });
    }

    // ส่งไฟล์
    res.download(file.file_path, file.file_name);
  } catch (error) {
    console.error('Error downloading lesson plan file:', error);
...
module.exports = {
  ...
  getAllStudentsLessonPlans,
  getAllStudentsTeachingSessions,
  downloadLessonPlanFile
};
