...
class TeachingSession {
  ...
  static async create(sessionData) {
    ...
    const query = `
      INSERT INTO teaching_sessions (
        ...
        class_level, class_room, student_count, lesson_topic, learning_activities,
  ...
  static async getBySchoolWithStudents(schoolId, page = 1, limit = 10, search = '', subject_id = '', status = '') {
    ...
    if (search) {
      whereConditions.push('(ts.lesson_topic LIKE ? OR u.first_name LIKE ? OR u.last_name LIKE ? OR s.subject_name LIKE ?)');
      const searchPattern = `%${search}%`;
      queryParams.push(searchPattern, searchPattern, searchPattern, searchPattern);
    }
    
    if (subject_id) {
      whereConditions.push('ts.subject_id = ?');
    ...
    try {
      const [teachingSessions] = await pool.execute(query, [...queryParams, limit, offset]);
