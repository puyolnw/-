...
class StudentApiService {
  ...
  async getStudentStatus(): Promise<{ success: boolean; data?: StudentStatus; message?: string }> {
    try {
      ...
      if (response.success) {
        ...
        try {
          const completionResponse = await apiService.get('/completion-requests/my-requests');
  ...
  async getSchoolTeachers(schoolId: string, academicYearId: number): Promise<{ success: boolean; data?: any; message?: string }> {
    try {
      console.log('🔵 Frontend - getSchoolTeachers called with:', { schoolId, academicYearId });
      
      const response = await apiService.get(`/student/assignments/school-teachers?school_id=${schoolId}&academic_year_id=${academicYearId}`);
      
      console.log('🔵 Frontend - getSchoolTeachers response:', response);
      return response;
    } catch (error: any) {
      console.error('🔴 Frontend - Error fetching school teachers:', error);
      return {
        success: false,
  ...
  // ลงทะเบียนเข้าโรงเรียนพร้อมเลือกครูพี่เลี้ยง
  async registerToSchool(schoolId: string, academicYearId: number, teacherId?: number): Promise<{ success: boolean; data?: any; message?: string }> {
    try {
      console.log('🔵 Frontend - registerToSchool called with:', { schoolId, academicYearId, teacherId });
