...
class SupervisorApiService {
  ...
  // ประเมินคำร้องขอสำเร็จการฝึกแบบละเอียด (เหมือนครูพี่เลี้ยง)
  async evaluateCompletionRequest(
    requestId: number, 
    comments: string, 
    rating: number, 
    status: string, 
    detailedEvaluationData?: any[]
  ): Promise<{ success: boolean; message?: string }> {
    try {
      console.log('🔵 Frontend - Supervisor evaluating completion request:', { requestId, comments, rating, status });
      const response = await apiService.put(`/supervisor/completion-requests/${requestId}/evaluate`, {
        supervisor_comments: comments,
        supervisor_rating: rating,
        status: status,
        detailed_evaluation_data: detailedEvaluationData
      });
      console.log('🔵 Frontend - Supervisor evaluation response:', response);
      return response;
    } catch (error: any) {
      console.error('Error evaluating completion request:', error);
      return {
        success: false,
        message: error.response?.data?.message || 'ไม่สามารถประเมินคำร้องได้'
      };
    }
  }
